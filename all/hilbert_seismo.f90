        ...
        subroutine hilbert_seismo(input,output,nx,nt)
         implicit none
...
! Hilbert transform
         do i=1,nx
            ...
            do j=1,nt
               input_fft(j)=cmplx(real(input(i,j)),0.0)
            enddo

            call sfftw_execute(plan1,input_fft,output_fft)

            do it=1,nt_fft/2
                output_fft(it)=cmplx(aimag(output_fft(it)),-real(output_fft(it)))
            enddo
            ...
            output(i,1:nt)=real(input_fft(1:nt)/nt_fft)
