...
!! top boundary
       mask_taper(:,-lv:geom_z(2)+5+npoints_pml)=0.0
       do i=-lv,nx+lv
          do j=1,12
             mask_taper(i,geom_z(2)+5+npoints_pml+j)=0.5-0.5*cos(pi*(j-1)/12.0)
          enddo
       enddo
...
!       do i=-lv,nx+lv
!          do j=1,12
!             mask_taper(i,nz_p+npoints_pml+j-1)=0.5+0.5*cos(pi*(j-1)/12)
!          enddo
!       enddo
...
!! hilbert receiver wavefield propagation
           ...
           call imaging_condition_udlr_eff(sou_p,p,p_hb,taper_x,taper_z,&
...
!! 
           ...
           call imaging_condition(sou_p,p_hb,image_hb,nx,nz,lv,lv)
...
!! Duverick method (combining with the implicit imaging condition)
           ...
           call imaging_condition_udlr_im(sou_p,p_hb,taper_x,taper_z,&
