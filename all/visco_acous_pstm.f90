...
!---
    ...
    include 'mpif.h'
     ...
     integer,allocatable::data_rec_id(:)
...
!       close(12)
       ...
       open(12,file='lhr_pstm_onobs_geom.txt')
       do i=1, nx_rec
          read(12,*) data_rec_id(i), geom_x(i), geom_z(i)
       enddo 
       close(12)

!! shot position: no meaning for stacked data
       isource=int(geom_x(1))+npoints_pml
       jsource=int(geom_z(1))+npoints_pml

!! receiver position
       do i=1,nx_rec
          rcv_x(i)=int(geom_x(i))+npoints_pml
          rcv_z(i)=int(geom_z(i))+npoints_pml
       enddo
       ...
       !! no need for forward modeling in the case of stacked data
       do it = 1,0
...
!       data_direc='/flush1/guo103/wave_decomp_csiro/bp_data/'
       file_p_rcv=trim(data_direc)//trim(file_p_part)

       open(file_p_rcv_no,file=file_p_rcv,access='direct',form='unformatted', &
            recl=ii_bi*nt)
        do i=1,nx_rec
          read(file_p_rcv_no,rec=data_rec_id(i)) (seis_rcv(i,j),j=1,nt)
        enddo
       close(file_p_rcv_no)
...
!              call snap_shot(sou_p,it,nx,nz,lv,nt_snap,recons_part,file_snap_no)     
            ...
            do j=nz_p1,nz_p2
               do i=nx_p1,nx_p2
                  image_illu(i,j)=image_illu(i,j)+p(i,j)*p(i,j)
               enddo
            enddo
