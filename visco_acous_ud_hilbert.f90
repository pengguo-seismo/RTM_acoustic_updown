...
!         sigma_zz
!
!!  this program was written by Peng Guo
!! 
...
!---
    ...
    real,allocatable::mask_taper(:,:)

    real,allocatable::sou_p_tmp(:,:),p_tmp(:,:)
    real,allocatable::sou_p_hb_tmp(:,:),p_hb_tmp(:,:)
...
!!   2: rtm, parameter file: parameter_rtm.txt
      ...
      allocate(mask_taper(-lv:nx+lv,-lv:nz+lv))
      mask_taper=1.0

      allocate(sou_p_tmp(-lv:nx+lv,-lv:nz+lv),&
                   p_tmp(-lv:nx+lv,-lv:nz+lv))

      allocate(sou_p_hb_tmp(-lv:nx+lv,-lv:nz+lv),&
                   p_hb_tmp(-lv:nx+lv,-lv:nz+lv))
...
!! for hilbert wavefield
       ...
       allocate(image_hb(1:nx,1:nz),image_duud_hb(1:nx,1:nz),image_dduu_hb(1:nx,1:nz))
       image_hb=0.0
       image_duud_hb=0.0
       image_dduu_hb=0.0
...
!!!!!!
       allocate(seis_rcv(1:nx_rec,1:nt))
...
!! top boundary
       mask_taper(:,-lv:int(geom_z(2))+5+npoints_pml)=0.0
       do i=-lv,nx+lv
          do j=1,10
             mask_taper(i,int(geom_z(2))+5+npoints_pml+j)=0.5-0.5*cos(pi*(j-1)/10.0)
          enddo
       enddo
...
!! bottom boundary 
       do i=-lv,nx+lv
          do j=1,10
             mask_taper(i,nz_p+npoints_pml+j-10)=0.5+0.5*cos(pi*(j-1)/10.0)
          enddo
       enddo

       mask_taper(:,nz_p+npoints_pml+1:nz+lv)=0.0

      open(12,file='mask_taper',access='direct',form='unformatted',&
            recl=ii_bi*(nz+2*lv+1))
        do j=-lv,nx+lv
          write(12,rec=j+lv+1) (mask_taper(j,i),i=-lv,nz+lv)
        enddo
      close(12)    
...
!!       bottom
           ...
           sou_p_hb_tmp=sou_p_hb*mask_taper
           p_hb_tmp=p_hb*mask_taper

!! wavefield decomposition
!! source wavefield decomposition
           call wavefield_decomp(sou_p_tmp,sou_p_hb_tmp,sou_p_up,sou_p_dw,sou_p_lf,sou_p_rt,&
                sou_p_lu,sou_p_ld,sou_p_ru,sou_p_rd,taper_x,taper_z,&
                nx,nz,lv,nxfft,nzfft,flag)

!! receiver wavefield decomposition 
           call wavefield_decomp(p_tmp,p_hb_tmp,rcv_p_up,rcv_p_dw,rcv_p_lf,rcv_p_rt,&
...
!!! an efficient up/down separation method by de-primary
           elseif (ud_flag.eq.2) then
...
!! hilbert receiver wavefield propagation
           ...
           call imaging_condition_udlr_eff(sou_p_tmp,p_tmp,p_hb_tmp,taper_x,taper_z,&
                image,image_du,image_ud,image_dd,image_uu,image_duud,image_dduu,&
                nx,nz,lv,nxfft,nzfft,flag)

!! implicit method
           elseif (ud_flag.eq.3) then

            call imaging_condition_udlr_im(sou_p_tmp,p_tmp,taper_x,taper_z,&
                 image_duud,image_dduu,nx,nz,lv,nxfft,nzfft,flag,it)

!! Duverick method (with laplace filter to remove low-wavenumbers)
           elseif(ud_flag.eq.4) then
!! 
           call iso_visco_step_p(vx_hb,vz_hb,p_hb,r_hb,memory_dvx_dx_hb,memory_dvz_dz_hb)
           ...
           call imaging_condition(sou_p_tmp,p_hb_tmp,image_hb,nx,nz,lv,lv)

!! Duverick method (combining with the implicit imaging condition)
           elseif(ud_flag.eq.5) then
           ...
           call imaging_condition_udlr_im(sou_p_tmp,p_hb_tmp,taper_x,taper_z,&
                 image_duud_hb,image_dduu_hb,nx,nz,lv,nxfft,nzfft,flag,it)
...
!! enddo time loop
       ...
       if(myid.eq.0) then
         open(81,file='cmpt_time_rtm'//char(i_shot/100+48)//&
               char(mod(i_shot,100)/10+48)//char(mod(i_shot,10)+48)//'.txt')
           write(81,*) t12_a
           write(81,*) t12_a_mp
         close(81)
       endif
       ...
       if(ud_flag.eq.4) then
          image_dduu=image_hb
!!  IM
        else if(ud_flag.eq.5) then
          image_dduu=image_duud_hb
          image_dd=image_hb
       endif

       image(nx_p1:nx_p1+1,:)=0.
       image(nx_p2-1:nx_p2,:)=0.
       image_ud(nx_p1:nx_p1+1,:)=0.
       image_ud(nx_p2-1:nx_p2,:)=0.
       image_du(nx_p1:nx_p1+1,:)=0.
       image_du(nx_p2-1:nx_p2,:)=0.
       image_uu(nx_p1:nx_p1+1,:)=0.
       image_uu(nx_p2-1:nx_p2,:)=0.
       image_dd(nx_p1:nx_p1+1,:)=0.
       image_dd(nx_p2-1:nx_p2,:)=0.
       image_duud(nx_p1:nx_p1+1,:)=0.
       image_duud(nx_p2-1:nx_p2,:)=0.
       image_dduu(nx_p1:nx_p1+1,:)=0.
       image_dduu(nx_p2-1:nx_p2,:)=0.
...
!     call mpi_barrier(mpi_comm_world,ierr)
    ...
    enddo   !! shot domain
     ...
     if(myid.eq.0) then 
       ...
       file_image=trim(file_image_part)//'_scale'//'_sum'
       ...
       image_01(nx_p1:nx_p2,nz_p1:nz_p2)=image_0(nx_p1:nx_p2,nz_p1:nz_p2)&
                                     /image_illu_0(nx_p1:nx_p2,nz_p1:nz_p2)

       image_duud_01(nx_p1:nx_p2,nz_p1:nz_p2)=image_duud_0(nx_p1:nx_p2,nz_p1:nz_p2)&
                                     /image_illu_0(nx_p1:nx_p2,nz_p1:nz_p2)

       image_dduu_01(nx_p1:nx_p2,nz_p1:nz_p2)=image_dduu_0(nx_p1:nx_p2,nz_p1:nz_p2)&
                                     /image_illu_0(nx_p1:nx_p2,nz_p1:nz_p2)

       image_du_01(nx_p1:nx_p2,nz_p1:nz_p2)=image_du_0(nx_p1:nx_p2,nz_p1:nz_p2)&
                                     /image_illu_0(nx_p1:nx_p2,nz_p1:nz_p2)

       image_ud_01(nx_p1:nx_p2,nz_p1:nz_p2)=image_ud_0(nx_p1:nx_p2,nz_p1:nz_p2)&
                                     /image_illu_0(nx_p1:nx_p2,nz_p1:nz_p2)

       image_uu_01(nx_p1:nx_p2,nz_p1:nz_p2)=image_uu_0(nx_p1:nx_p2,nz_p1:nz_p2)&
                                     /image_illu_0(nx_p1:nx_p2,nz_p1:nz_p2)

       image_dd_01(nx_p1:nx_p2,nz_p1:nz_p2)=image_dd_0(nx_p1:nx_p2,nz_p1:nz_p2)&
                                     /image_illu_0(nx_p1:nx_p2,nz_p1:nz_p2)
